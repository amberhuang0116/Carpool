	...
	public void createButPanel() {
		...
		class ButtonListener implements ActionListener {
			public void actionPerformed(ActionEvent event) {
				...
				int vacanciesAvailable = Integer.parseInt(vacancy.getText());
				String carsModel = carModel.getText();
				int lowPrice = slider.getValue();
				int highPrice = ((RangeSlider) slider).getUpperValue();

				try {
					boolean add = exec.addDriverGroup(lowPrice, highPrice, 0, vacanciesAvailable, start, destination, carsModel, date);
					if(add==true) {
